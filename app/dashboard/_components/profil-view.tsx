...
export function ProfilView() {
  const { user, hierarchy, profileStatus, memberStatus } = useDashboardData();
        ...
        <div className="space-y-4 p-5">
          ...
          <div className="flex items-start gap-3 rounded-xl border border-border/70 p-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm">{user.roles.join(", ") || "MEMBER"}</p>
              <p className="mt-1 text-xs text-muted-foreground">
                Status portal: {profileStatus ?? "—"} • Status anggota: {memberStatus ?? "—"}
              </p>
            </div>
          </div>
