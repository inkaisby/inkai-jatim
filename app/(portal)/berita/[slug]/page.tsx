...
export default async function BeritaDetailPage({
  ...
  return (
    <main>
      ...
      <div className="mb-6">
        <BackLink href="/berita" label="Kembali ke Berita" />
      </div>
        ...
        res.data ? (
          <article className="surface p-8">
            ...
            <div className="mt-2 text-sm text-muted-foreground">
              {res.data.published_at ? new Date(res.data.published_at).toLocaleString("id-ID") : ""}
            </div>
            {res.data.excerpt ? (
              <p className="mt-5 text-base text-muted-foreground">{res.data.excerpt}</p>
            ) : null}
            {res.data.content ? (
              ...
              <p className="mt-6 text-sm text-muted-foreground">(Konten belum diisi.)</p>
            )}
          </article>
        ) : null
      ) : (
